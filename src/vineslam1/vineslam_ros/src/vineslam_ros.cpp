...
void VineSLAM_ros::imuListener(const geometry_msgs::msg::Vector3Stamped::SharedPtr msg)
{
  ...
  input_data_.imu_pose_.Y_ = 0;  // TODO (André Aguiar): For now we do not use this because it requires proper calibration

  tf2::Quaternion q;
  q.setRPY(input_data_.imu_pose_.R_, input_data_.imu_pose_.P_, robot_pose_.Y_);
  ...
  mag_pose.pose.orientation.z = q.x();
  mag_pose.pose.orientation.y = q.y();
  mag_pose.pose.orientation.z = q.z();
  mag_pose.pose.orientation.w = q.w();
...
void VineSLAM_ros::imuDataListener(const sensor_msgs::msg::Imu::SharedPtr msg)
{
  ...
  if (init_gyro_)
  {
    ...
    input_data_.p_imu_data_pose_ = Pose(0, 0, 0, 0, 0, 0);
    init_gyro_ = false;
    return;
  }
  ...
  input_data_.imu_data_pose_.R_ += msg->angular_velocity.x * d;
  input_data_.imu_data_pose_.P_ += msg->angular_velocity.y * d;
  input_data_.imu_data_pose_.Y_ += msg->angular_velocity.z * d;

  p_imu_observation_timestamp_ = c_imu_observation_timestamp;

  tf2::Quaternion q;
  q.setRPY(input_data_.imu_data_pose_.R_, input_data_.imu_data_pose_.P_, input_data_.imu_data_pose_.Y_);
  ...
  gyro_pose.pose.orientation.z = q.x();
  gyro_pose.pose.orientation.y = q.y();
  gyro_pose.pose.orientation.z = q.z();
  gyro_pose.pose.orientation.w = q.w();
...
void VineSLAM_ros::computeInnovation(const Pose& wheel_odom_inc, const Pose& imu_rot_inc, Pose& output_pose)
{
  ...
  output_pose = Pose(wheel_odom_inc.x_, wheel_odom_inc.y_, 0, imu_rot_inc.R_, imu_rot_inc.P_, imu_rot_inc.Y_);
}
