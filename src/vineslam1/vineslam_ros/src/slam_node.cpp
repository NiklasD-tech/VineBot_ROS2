...
void SLAMNode::loadParameters(Parameters& params)
{
  ...
  param = prefix + ".heading_offset";
  this->declare_parameter(param);
  if (!this->get_parameter(param, params.heading_offset_))
  {
    RCLCPP_WARN(this->get_logger(), "%s not found.", param.c_str());
  }
  param = prefix + ".publish_level";
  this->declare_parameter(param);
  ...
  param = prefix + ".multilayer_mapping.topological_map.autogen_topological_map";
  this->declare_parameter(param);
  if (!this->get_parameter(param, params.autogen_topological_map_))
  {
    RCLCPP_WARN(this->get_logger(), "%s not found.", param.c_str());
  }
  param = prefix + ".multilayer_mapping.topological_map.folder";
  this->declare_parameter(param);
  if (!this->get_parameter(param, params.topological_map_folder_))
  {
    RCLCPP_WARN(this->get_logger(), "%s not found.", param.c_str());
  }
  param = prefix + ".multilayer_mapping.topological_map.dimensions.x";
  this->declare_parameter(param);
  if (!this->get_parameter(param, params.topological_map_dim_x_))
  {
    RCLCPP_WARN(this->get_logger(), "%s not found.", param.c_str());
  }
  param = prefix + ".multilayer_mapping.topological_map.dimensions.y";
  this->declare_parameter(param);
  if (!this->get_parameter(param, params.topological_map_dim_y_))
  {
    RCLCPP_WARN(this->get_logger(), "%s not found.", param.c_str());
  }
  param = prefix + ".multilayer_mapping.topological_map.dimensions.square_size";
  this->declare_parameter(param);
...
void SLAMNode::getTfs()
{
  ...
  while (!got_laser2base && rclcpp::ok())
  {
    try
    {
      laser2base_msg = tf_buffer.lookupTransform(params_.lidar_sensor_frame_, params_.base_frame_id_, rclcpp::Time(0));
    }
...
void SLAMNode::process()
{
  ...
  {
    Tf p_imu_tf = input_data_.p_imu_data_pose_.toTf();
    Tf c_imu_tf = input_data_.imu_data_pose_.toTf();
    Tf imu_inc_tf = p_imu_tf.inverse() * c_imu_tf;
    Pose imu_inc(imu_inc_tf.R_array_, imu_inc_tf.t_array_);
...
void SLAMNode::broadcastTfsWithTopic()
{
  ...
  base2map_msg.header.stamp = header_.stamp;
  base2map_msg.header.frame_id = "odom";
  base2map_msg.child_frame_id = params_.base_frame_id_;
...
void SLAMNode::broadcastTfs()
{
  ...
  {
    // Get odom to base transformation
    odom2base_msg = tf_buffer.lookupTransform("odom", params_.base_frame_id_, rclcpp::Time(0), rclcpp::Duration(300000000));
...
void SLAMNode::genTopologicalMap()
{
  ...
  std::filesystem::create_directories(params_.grid_map_files_folder_);
  std::filesystem::create_directories(params_.map_output_folder_);
