...
void PF::motionModel(const Pose& odom_inc)
{
  ...
  for (auto& particle : particles_)
  {
    ...
    Pose pose_noise(params_.sigma_xx_, params_.sigma_yy_, params_.sigma_zz_, params_.sigma_RR_, params_.sigma_PP_, params_.sigma_YY_);
    ...
    pose_noise.x_ = params_.sigma_xx_ * d_trans * sampleGaussian(1.0);                 // xx
    pose_noise.y_ = params_.sigma_yy_ * d_trans * sampleGaussian(1.0);                 // yy
    ...
    pose_noise.Y_ = params_.sigma_YY_ * std::fabs(odom_inc.Y_) * sampleGaussian(1.0);  // YY
