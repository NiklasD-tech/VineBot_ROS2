...
namespace vineslam
{
// Declare header of kernel particle filter function ("vineslam/kernels/localization/pf.cu")
void pfUpdate(const std::vector<Corner>& corners, const std::vector<Planar>& planars, std::vector<Particle>& particles, LocalizationMappingInterface* localization_mapping_interface);
...
#if USE_CUDA
#if VERBOSE == 1
    t_->tick("(GPU) pf::update()");
#endif
    pfUpdate(obsv.corners_, obsv.planars_, pf_->particles_, localization_mapping_interface_);
#if VERBOSE == 1
    t_->tock();
#endif
#if VERBOSE == 1
    t_->tick("(CPU) pf::update()");
#endif
    pf_->update(obsv.planes_, obsv.ground_plane_, obsv.gps_pose_, obsv.gps_heading_, obsv.imu_pose_, topological_map);
#if VERBOSE == 1
    t_->tock();
#endif
#else
