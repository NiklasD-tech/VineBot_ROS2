...
folium.GeoJson("Vineyard2.geojson", 
               name="Vineyard2",
               style_function=lambda x:borderStyle).add_to(m)
folium.GeoJson("Vineyard3.geojson", 
               name="Vineyard3",
               style_function=lambda x:borderStyle).add_to(m)
...
def on_button_click(selected_file):

    points = read_geojson(selected_file)
    print("Points in", selected_file, ":", points)

geojson_files = ["Vineyard1.geojson", "Vineyard2.geojson"]  # Passe diese Liste entsprechend deinen Dateinamen an

dropdown_options = ''.join(f'<option value="{filename}">{filename}</option>' for filename in geojson_files)
# dropdown_html = f"""
# <div>
#     <select id="geojson-dropdown">
#         <option value="" disabled selected>Choose GeoJSON file</option>
#         {dropdown_options}
#     </select>
...
def on_dropdown_change(change):
    ...
    if selected_file:
        on_button_click(selected_file)

import ipywidgets as widgets

dropdown = widgets.Dropdown(options=geojson_files, description='Choose GeoJSON file:')
