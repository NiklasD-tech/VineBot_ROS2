...
def on_dropdown_change(change):
    ...
    if selected_file:
        ...
        #ausgewähltes grün
        folium.Polygon(
            locations=switch_lat_long(points),
            color="red",
            weight=3,
            fill_color="green",
            fill_opacity=0.5,
        ...
        m.save("map.html")  # Karte speichern
        display(m)    
        display(dropdown)
...
def read_files():
    folder_path = "/home/irobot/ros2_ws/src/geofence/geofence/Vineyards"
    geojsonNames=[]
    wineyardenames=[]
    ...
    return geojsonNames,wineyardenames
...
if __name__ == '__main__':
    ...
    geojsonNames,wineyardenames=read_files()
    ...
    geojsonNames.sort()
    wineyardenames.sort()
    dropdown_options = ''.join(f'<option value="{filename[:-8]}">{filename[:-8]}</option>' for filename in geojsonNames)
